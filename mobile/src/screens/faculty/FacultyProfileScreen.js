...
const FacultyProfileScreen = ({ navigation }) => {
  ...
  const generatePublicLink = async () => {
    try {
      const response = await api.get('/users/public-schedule-link');
      setPublicLink(response.data.publicUrl);
      Alert.alert('Success', 'Public schedule link generated!', [
        {
          text: 'View',
          onPress: () => navigation.navigate('PublicSchedule', { token: response.data.token }),
        },
        { text: 'OK' },
      ]);
    } catch (error) {
      Alert.alert('Error', 'Failed to generate public link');
    }
  };
      ...
      <View style={styles.content}>
        ...
        <View style={[styles.section, { backgroundColor: colors.card }]}>
          ...
          >
            ...
            <Text style={[styles.actionText, { color: colors.text }]}>Generate Public Link</Text>
            <Ionicons name="chevron-forward" size={20} color={colors.textSecondary} />
          </TouchableOpacity>
